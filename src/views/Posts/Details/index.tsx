...
export default function DetailPosts() {
  ...
  const hasIncreaseViewCount = useRef(false); // 조회수 증가 참조 상태
        ...
        <div className='detail-posts-bottom__comment-box'>
          <div className='detail-posts-bottom__comment-container'>
            ...
            <div className='detail-posts-bottom__comment-list-container'>
              {viewList.map(item => <CommentItem commentListItem ={item} onDeleteComment={onDeleteCommentButtonClickHandler} currentUserNickname={loginUser?.nickname || ''}/>)}
            </div>
          </div>
  ...
  useEffect(() => {
    if(!postsNumber) return;
    if(!hasIncreaseViewCount.current) {
      hasIncreaseViewCount.current = true;
